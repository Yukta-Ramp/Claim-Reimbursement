...
export const ClaimCreationPage: React.FC = () => {
  const { user } = useAuth();
  ...
  const [claimType, setClaimType] = useState<ClaimType>('General');
  ...
  const handleSaveBill = (billData: Partial<Bill>) => {
    if (editingBill) {
      ...
      // Add new bill
      const { id, ...billDataWithoutId } = billData as Bill;
      const newBill: Bill = {
        id: `bill-${Date.now()}`,
        ...billDataWithoutId,
      };
              ...
              <CardContent className="pt-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={claimType}
                      onValueChange={(value: string) => setClaimType(value as ClaimType)}
                    >
